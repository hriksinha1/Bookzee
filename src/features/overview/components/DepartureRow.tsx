    ...
    <div className="py-3 px-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 hover:bg-[#FAF9F6] transition-colors">
      ...
      <div className="space-y-0.5 min-w-0">
        <div className="flex items-baseline gap-2">
          ...
          {isCheckedOut && (
            <span className="text-[11px] font-medium text-[#5C6E6B]">
              Checked out
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-1.5">
          {!isCheckedOut ? (
            <>
              {hasDue ? (
                <>
                  {/* If money is outstanding: Record payment is primary, Check out is secondary */}
                  <button
                    type="button"
                    onClick={() => onRecordPayment(booking, fin.amountDue)}
                    className="px-2.5 py-1.5 rounded-lg bg-[#C45532] text-white text-xs font-medium hover:bg-[#A84323] transition-colors inline-flex items-center gap-1 shadow-2xs"
                  >
                    <CreditCard size={12} />
                  ...
                  <button
                    ...
                    className="px-2.5 py-1.5 rounded-lg border border-[#D8D2C5] text-xs font-medium text-[#1A2B28] hover:bg-stone-100 transition-colors"
                  >
                    Check out
                  </button>
                </>
              ) : (
                /* If paid in full: Check out is primary */
                <button
                  type="button"
                  onClick={(e) => onCheckOut(booking, e)}
