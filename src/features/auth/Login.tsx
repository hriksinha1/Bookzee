...
export default function Login({ onLogin }: { onLogin: () => void }) {
  const [email, setEmail] = useState('host@serene.local');
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-[#F7F5F0] text-[#18312F]">
      {/* Left Hospitality Atmosphere Showcase */}
      <div className="lg:w-7/12 relative bg-[#0B3F3A] text-white p-8 sm:p-12 lg:p-16 flex flex-col justify-between overflow-hidden">
        {/* Subtle Architectural Gradient Backing */}
        <div className="absolute inset-0 opacity-15 bg-[radial-gradient(#0F766E_1px,transparent_1px)] [background-size:20px_20px] pointer-events-none" />
        <div className="absolute -top-32 -left-32 w-96 h-96 rounded-full bg-[#0F766E] opacity-25 blur-3xl pointer-events-none" />
        <div className="absolute -bottom-32 -right-32 w-96 h-96 rounded-full bg-[#C65D3A] opacity-20 blur-3xl pointer-events-none" />

        {/* Top Branding */}
        <div className="relative z-10">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-[#0F766E] to-[#155E75] flex items-center justify-center shadow-md">
            ...
            <div>
              <span className="font-semibold text-xl tracking-tight text-white flex items-center gap-2">
                MyTrackYo
                <span className="text-[10px] font-bold tracking-widest px-2 py-0.5 rounded-full bg-[#E6F3F1]/15 text-[#E6F3F1]">
                  2.0
                </span>
              </span>
              <p className="text-xs text-white/70">Property Management for Homestays & Boutique Hotels</p>
            </div>
          </div>
        </div>
        ...
        <div className="relative z-10 my-12 lg:my-0 max-w-xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 text-white text-xs font-medium backdrop-blur-xs mb-6">
            <Sparkles size={14} className="text-[#F4EBDD]" />
            <span>Designed for hospitality operators</span>
          </div>
      ...
      <div className="lg:w-5/12 flex items-center justify-center p-6 sm:p-12 lg:p-16">
        <div className="w-full max-w-md space-y-8">
          <div>
            ...
            <h2 className="text-2xl sm:text-3xl font-semibold text-[#18312F] mt-1 tracking-tight">
              Welcome back
            </h2>
            <p className="text-sm text-[#5F716E] mt-2 leading-relaxed">
              Manage your stays, guests, and payment collections across your properties from one place.
            </p>
          </div>

          {/* Direct One-Click Demo Button */}
          <div className="p-5 rounded-2xl bg-[#E6F3F1] border border-[#BDE4CD] space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] px-2 py-0.5 rounded-full bg-[#2F7D5A] text-white font-medium">
                Ready to Explore
              </span>
            </div>
            <p className="text-xs text-[#18312F] leading-relaxed">
            ...
            <button
              ...
              disabled={loading}
              className="w-full py-3 px-4 rounded-xl bg-[#0F766E] text-white font-medium hover:bg-[#115E59] active:scale-[0.99] transition-all flex items-center justify-center gap-2 shadow-sm text-sm"
            >
              {loading ? (
          ...
          <div className="relative flex items-center justify-center">
            <div className="border-t border-[#D4DED9] w-full" />
            <span className="bg-[#F7F5F0] px-3 text-xs text-[#8B9B97] uppercase tracking-wider">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="block text-xs font-semibold text-[#18312F]">Password</label>
                <span className="text-xs text-[#0F766E] hover:underline cursor-pointer">
                  Forgot password?
                </span>
              </div>
              <input
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 px-4 rounded-xl bg-[#18312F] text-white font-medium hover:bg-[#0B3F3A] transition-all text-sm flex items-center justify-center gap-2 mt-2"
            >
              Sign In to Dashboard
            </button>
          </form>

          <p className="text-center text-xs text-[#8B9B97]">
            MyTrackYo PMS • Enterprise Grade Security for Hospitality
          </p>
        </div>
      </div>
    </div>
