    ...
    return (
      <div className="animate-pulse space-y-6 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[1, 2, 3, 4].map((n) => (
            <div key={n} className="h-24 bg-white rounded-2xl border border-[#D4DED9]"></div>
          ))}
        </div>
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        >
          <AlertCircle size={15} className="text-[#B7791F]" />
          <span>View Outstanding Balances</span>
        </Link>
      </div>

      {/* Summary Metrics Strip (Requirement #23) */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="card p-4 bg-white">
          <span className="text-xs text-[#5F716E]">Net Collected</span>
          <div className="text-xl sm:text-2xl font-bold text-[#2F7D5A] mt-1">
            {fmtINR(metrics.collected)}
          </div>
        </div>
        <div className="card p-4 bg-white">
          <span className="text-xs text-[#5F716E]">Completed Receipts</span>
          <div className="text-xl sm:text-2xl font-bold text-[#18312F] mt-1">
            {metrics.completedCount}
          </div>
        </div>
        <div className="card p-4 bg-white">
          <span className="text-xs text-[#5F716E]">Refunds Processed</span>
          <div className="text-xl sm:text-2xl font-bold text-[#B84A4A] mt-1">
            {metrics.refundedCount}
          </div>
        </div>
        <div className="card p-4 bg-white">
          <span className="text-xs text-[#5F716E]">Refunded Amount</span>
          <div className="text-xl sm:text-2xl font-bold text-[#B84A4A] mt-1">
                ...
                {filteredPayments.map((p) => (
                  ...
                  >
                    <td className="py-3.5 px-4">
                      <div className="font-mono font-semibold text-[#18312F] group-hover:text-[#0F766E] transition-colors">
                        {p.payment_no}
                      </div>
                    ...
                    <td className="py-3.5 px-4 text-[#5F716E] whitespace-nowrap">
                      {fmtDate(p.date)}
                    </td>
                    <td className="py-3.5 px-4">
                    ...
                    <td className="py-3.5 px-4">
                      <div className="font-medium text-[#18312F]">{p.method}</div>
                      {p.ref_id && (
                        <div className="text-[10px] text-[#8B9B97] font-mono truncate max-w-[120px]">
                          Ref: {p.ref_id}
                        </div>
                      )}
                    </td>
                    <td className="py-3.5 px-4 text-[#5F716E]">{p.purpose}</td>
                    <td className="py-3.5 px-4 text-right font-bold text-sm">
                      <span
                        className={p.status === 'Refunded' ? 'text-[#B84A4A]' : 'text-[#2F7D5A]'}
                      >
                        {p.status === 'Refunded' ? '-' : '+'}
                        {fmtINR(p.amount)}
                      </span>
